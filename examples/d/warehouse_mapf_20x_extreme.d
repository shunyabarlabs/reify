...
//
// ============================================================================
//  Autonomous Warehouse Robot Fleet MAPF 20x Extreme Benchmark
// ============================================================================
//
//  Models dynamic spatio-temporal pathfinding for 7 Autonomous Mobile Robots (AMRs)
//  navigating a 7x7 grid warehouse floor over a 9-tick operational horizon.
//
//  Hard CNF Scale:
//  - 7 Autonomous Mobile Robots (AMRs)
//  - 7x7 Grid Workspace (49 spatial cells)
//  - 9 Time Ticks
//  - 3,087 Logical Decision Variables
//  - ~380,000 Pure CNF Clauses
...
//  - Start Depots at t=0 and Workstation Target Goals at t=8
//
// ============================================================================

module warehouse_mapf_20x_extreme;
...
enum GRID_DIM = 7;
enum NUM_CELLS = GRID_DIM * GRID_DIM; // 49 cells
enum NUM_ROBOTS = 7;
enum NUM_TICKS = 9;
...
struct RobotRouteConfig {
    size_t startX, startY;
    size_t goalX, goalY;
}

RobotRouteConfig[NUM_ROBOTS] getRobotConfigs() {
    return [
        RobotRouteConfig(0, 0, 4, 4),
        RobotRouteConfig(1, 0, 5, 3),
        RobotRouteConfig(2, 0, 6, 2),
        RobotRouteConfig(3, 0, 0, 4),
        RobotRouteConfig(4, 0, 1, 3),
        RobotRouteConfig(5, 0, 2, 2),
        RobotRouteConfig(6, 0, 3, 1)
    ];
}

void main() {
    writeln("==========================================================================");
    writeln("  Autonomous Warehouse Robot Fleet MAPF 20x Extreme Benchmark");
    writeln("==========================================================================");
    ...
    auto model = new Model("warehouse-mapf-20x-extreme");
    ...
    auto configs = getRobotConfigs();
    foreach (r; 0 .. NUM_ROBOTS) {
        size_t startCell = cellIndex(configs[r].startX, configs[r].startY);
        model.require(
            format("start_depot_r%d", r),
            pos[r][startCell][0]
        );

        size_t goalCell = cellIndex(configs[r].goalX, configs[r].goalY);
        model.require(
